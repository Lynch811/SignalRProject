...
namespace SignalR.DataAccessLayer.EntityFramework
{
    public class EfProductDal : GenericRepository<Product>, IProductDal
    {
		...
		{
			using var context =new SignalRContext();
			return context.Products.Where(x=>x.CategoryID==(context.Categories.Where(y=>y.CategoryName=="İçecek").Select(z=>z.CategoryID).FirstOrDefault()))
				.Count();
		}
		...
		{
			using var context = new SignalRContext();
			return context.Products.Where(x => x.CategoryID == (context.Categories.Where(y => y.CategoryName == "Hamburger").Select(z => z.CategoryID).FirstOrDefault()))
				.Count();
		}
		...
		{
			using var context = new SignalRContext();
			var mostExpensiveProduct=context.Products.OrderByDescending(p=>p.Price).FirstOrDefault();
		...
		{
			using var context = new SignalRContext();
			var cheapestProduct=context.Products.OrderBy(p=>p.Price).FirstOrDefault();
